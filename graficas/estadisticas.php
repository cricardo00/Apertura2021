...
$sqlap = "SELECT count(*) AS FORMACION FROM aprendiz WHERE tecnoacademia = $tecousu AND estadouno = 'EN FORMACION'";
$resultap = $mysqli->query($sqlap);

while($rowap = $resultap->fetch_assoc()) {
	 $taprendicesact .= $rowap['FORMACION'];
}

// Total Aprendices Inactivos
$sqlap = "SELECT count(*) AS DESERTADO FROM aprendiz WHERE tecnoacademia = $tecousu AND estadouno = 'DESERTADO'";
$resultap = $mysqli->query($sqlap);

while($rowap = $resultap->fetch_assoc()) {
	 $taprendicesinact .= $rowap['DESERTADO'];
}
...
$sqlap = "SELECT count(*) AS ING FROM aprendiz WHERE tecnoacademia = $tecousu AND lineauno = 'INGENIERIA'";
$resultap = $mysqli->query($sqlap);
...
$sqlap = "SELECT count(*) AS TICS FROM aprendiz WHERE tecnoacademia = $tecousu AND lineauno = 'TICS'";
$resultap = $mysqli->query($sqlap);
...
$sqlap = "SELECT count(*) AS NANO FROM aprendiz WHERE tecnoacademia = $tecousu AND lineauno = 'NANOTECNOLOGIA'";
$resultap = $mysqli->query($sqlap);
...
$sqlap = "SELECT count(*) AS BIO FROM aprendiz WHERE tecnoacademia = $tecousu AND lineauno = 'BIOTECNOLOGIA'";
$resultap = $mysqli->query($sqlap);
...
$sqlap = "SELECT count(*) AS CB FROM aprendiz WHERE tecnoacademia = $tecousu AND lineauno = 'CIENCIAS BASICAS'";
$resultap = $mysqli->query($sqlap);
...
Highcharts.chart('containerTA', {
    ...
    title: {
        text: 'Total Aprendices Tecnoacademia 2020'
    },
    tooltip: {
    ...
    series: [{
        ...
        colorByPoint: true,
        data: [{
            name: 'Aprendices Formacion <?php echo $taprendicesact; ?>',
            y: <?php echo $activos; ?>,
            sliced: true,
            selected: true,
        }, {
            name: 'Aprendices Desertados <?php echo $taprendicesinact; ?>',
...
Highcharts.chart('containerAxG', {
    ...
    title: {
        text: 'Total Aprendices Tecnoacademia por Genero 2020'
    },
    tooltip: {
        pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>'
    },
...
Highcharts.chart('containerAxLTA', {
    ...
    title: {
        text: 'Total Aprendices Tecnoacademia por Linea 2020'
    },
    tooltip: {
        pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>'
    },
